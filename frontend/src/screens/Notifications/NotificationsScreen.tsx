...
import { View, Text, StyleSheet, FlatList, Image, TouchableOpacity, Platform, TouchableWithoutFeedback } from 'react-native';
...
const NotificationScreen = () => {
  ...
  useEffect(() => {
    const fetchPostsAndUser = async () => {
      try {
        ...
        if (token) {
          try {
            const userRes = await apiClient.post(`/api/auth/userdata`, {
              token,
            });
  ...
  const toggleLike = async (postId: string, forceLike = false) => {
    ...
    try {
      ...
      const response = await apiClient.put(`/api/posts/${postId}/likes`, {});
      if (response.data.status !== "Ok") {
  ...
  return (
    <SafeAreaView edges={['top', 'left', 'right']} style={styles.outer_container}>
