...
const BibleComponent = () => {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        // Fetch Languages
        try {
          const langRes = await apiClient.get(`/api/bible/languages`);
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      ...
      try {
        const res = await apiClient.get(`/api/bible/books`, { params: { language } });
  ...
  const deductCredit = async () => {
    ...
    try {
      const response = await apiClient.post(`/api/users/deduct-credit`, {});
      if (response.data.status === 'Ok') {
          ...
          onPress: async () => {
            try {
              ...
              if (language.toLowerCase() !== 'english') {
                try {
                  const enRes = await apiClient.get(`/api/bible/verse`, {
  ...
  const fetchCompareVerse = async (lang: string) => {
    ...
    try {
      const res = await apiClient.get(`/api/bible/verse`, {
