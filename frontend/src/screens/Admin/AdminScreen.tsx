...
import { View, Text, ScrollView, Pressable, TextInput, StyleSheet, Alert, ActivityIndicator, Platform, TouchableOpacity } from 'react-native';
...
const AdminScreen = () => {
  ...
  const fetchPendingRequests = async () => {
    try {
      const res = await apiClient.get(`/api/pending-rent-requests`);
  ...
  const fetchRequestHistory = async () => {
    try {
      const res = await apiClient.get(`/api/request-history`);
  ...
  const fetchAnalytics = async () => {
    try {
      const res = await apiClient.get(`/api/book-analytics`);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await apiClient.post(`/api/approve-rent-request`, { userEmail, book_id });
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await apiClient.post(`/api/reject-rent-request`, { userEmail, book_id });
  ...
  const handleAddBook = async () => {
    try {
      const res = await apiClient.post(`/api/add-book`, newBook);
