...
// (@firebase/auth/dist/**/index.rn.d.ts). Metro resolves that build, but tsc
// follows the package's default "browser" typings, which omit it -- so the
// import type-errors even though the symbol exists at runtime. Requiring it
// through the same specifier keeps Metro's resolution and sidesteps the wrong
// type surface, rather than silencing the error with a blanket `any` import.
