...
const UserProfileScreen = () => {
  ...
  const deleteImageFromCloudinary = async (imageUrl: any) => {
    try {
      ...
      const token = await AsyncStorage.getItem('token');
      const response = await apiClient.post(`/api/cloudinary/delete`, {
        token,
        publicId: publicId
      });
  ...
  const uploadImage = async (uri: any) => {
    ...
    try {
      ...
      const uploaded = await uploadToCloudinary(uri, 'profile');
  ...
  const saveProfile = async () => {
    try {
      ...
      const response = await apiClient.put(`/api/users/update`, updatedData);
      if (response.data.status === 'Ok') {
