...
// ════════════════════════════════════════════════
import { MessageNote, NoteCategory, ReminderNote, VerseHighlight, VoiceNote } from './types';
...
export const getAllNotes = async (isPublic = false): Promise<MessageNote[]> => {
    try {
        ...
        if (isPublic) {
            ...
            res = await apiClient.get(`/api/notes/public`);
        } else {
            const token = await AsyncStorage.getItem('token');
            if (!token) return []; // Guest has no token — skip my-notes fetch
            const headers = await getAuthHeaders();
            res = await apiClient.get(`/api/notes/my`, headers);
        }
...
export const createNote = async (
    ...
    try {
        ...
        const res = await apiClient.post(`/api/notes`, { ...data, title }, headers);
...
export const updateNote = async (
    ...
    try {
        ...
        const res = await apiClient.put(`/api/notes/${id}`, data, headers);
        if (res.data.status === 'ok') {
            return res.data.data;
        }
        return null;
...
export const deleteNote = async (id: string): Promise<void> => {
    try {
        const headers = await getAuthHeaders();
        await apiClient.delete(`/api/notes/${id}`, headers);
    } catch (err: any) {
        if (err?.response?.status === 401) await handle401();
...
export const getStandaloneReminders = async (): Promise<ReminderNote[]> => {
    try {
        ...
        if (!token) return [];
        const headers = await getAuthHeaders();
        const res = await apiClient.get(`/api/reminders`, headers);
        if (res.data.status === 'ok') {
            const data = res.data.data || [];
...
export const deleteStandaloneReminder = async (rmId: string): Promise<void> => {
    ...
    try {
        const headers = await getAuthHeaders();
        await apiClient.delete(`/api/reminders/${rmId}`, headers);
    } catch (err: any) {
        if (err?.response?.status === 401) await handle401();
...
export const clearAllStandaloneReminders = async (): Promise<void> => {
    ...
    try {
        const headers = await getAuthHeaders();
        await apiClient.delete(`/api/reminders/all`, headers);
        const key = await getStandaloneRemindersKey();
...
export const clearAllNotesData = async (): Promise<{ deleted: number }> => {
    ...
    try {
        const headers = await getAuthHeaders();
        const res = await apiClient.delete(`/api/notes/all`, headers);
