...
export default function AuthorBooks() {
  ...
  const fetchAuthor = async () => {
    try {
      ...
      const res = await apiClient.get(`/api/authors/${authorId}`);
      console.log('Author API Response:', res.data);
      if (res.data.status === 'Ok') {
        setAuthor(res.data.data);
      } else {
        console.warn('Author not found, falling back to books data');
        const booksRes = await apiClient.get(`/api/authors/${authorId}/books`);
  ...
  const fetchAuthorBooks = async () => {
    try {
      ...
      const res = await apiClient.get(`/api/authors/${authorId}/books`);
