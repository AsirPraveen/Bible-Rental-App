...
const GameDeck = ({ navigation }: any) => {
  ...
  const saveDeck = async () => {
    ...
    try {
      ...
      const res = await apiClient.post(`/api/game/deck/equip`, payload);
  ...
  const handleEquipArmor = async (armorName: string) => {
    ...
    setLoading(true);
    try {
      const res = await apiClient.post(`/api/game/deck/equip-armor`, {
        email,
    ...
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient colors={colors.linearGradient} style={styles.container}>
        ...
        <ScreenHeader
          ...
          title="Deck Builder"
          backIcon={<MaterialCommunityIcons name="arrow-left" size={28} color="#BFDBFE" />}
          right={
