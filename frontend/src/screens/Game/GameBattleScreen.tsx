...
import { View, Text, StyleSheet, TouchableOpacity, ProgressBarAndroid, Platform, Animated, Modal, TextInput, ScrollView, Image } from 'react-native';
...
const GameBattle = ({ route, navigation }: any) => {
  ...
  useEffect(() => {
    const fetchBattleData = async () => {
      try {
        ...
        const resGame = await apiClient.get(`/api/game/data?email=${resUser.data.data.email}`);
  ...
  const saveLevelCompletion = async () => {
    try {
      ...
      await apiClient.post(`/api/game/level/complete`, {
    ...
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient colors={colors.theme === 'dark' ? currentTheme : colors.linearGradient} style={styles.container}>
        ...
        <ScreenHeader
          ...
          title="Battle Arena"
          backIcon={<MaterialCommunityIcons name="arrow-left" size={28} color="#FECACA" />}
          spacerWidth={28}
        />
        ...
        <Modal
          navigationBarTranslucent
          statusBarTranslucent visible={showSwitchModal} transparent animationType="fade">
           <View style={styles.modalBg}>
        ...
        <Modal
          navigationBarTranslucent
          statusBarTranslucent visible={endGameState !== 'none'} transparent animationType="fade">
           <View style={styles.modalBg}>
