...
/** Which chapters the user has read, keyed `${book}-${chapter}`. */
type CompletedChapters = Record<string, boolean>;
...
const ReadingTrackerComponent = () => {
  ...
  const saveCompletedChapters = async (chapters: CompletedChapters) => {
    try {
  ...
  const submitInitialTreasures = async (count: number | string) => {
  ...
  const toggleChapterComplete = (book: string, chapter: number) => {
    const key = `${book}-${chapter}`;
    const updated = { ...completedChapters, [key]: !completedChapters[key] };
    saveCompletedChapters(updated);
  };

  const toggleAllChaptersInBook = (book: string, chapterCount: number) => {
  ...
  const getTotalProgressInternal = (chapters: CompletedChapters) => {
  ...
  const getTestamentProgress = (testament: keyof typeof BIBLE_STRUCTURE) => {
  ...
  const getProgressColor = (percentage: number, colors: ColorsType) => {
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.container}>
            {Object.entries(BIBLE_STRUCTURE).map(([testament, books]) => {
              const testamentProgress = getTestamentProgress(
                  testament as keyof typeof BIBLE_STRUCTURE,
                );
