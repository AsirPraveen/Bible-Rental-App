...
import { View, Text, StyleSheet, Platform, TouchableOpacity, ScrollView, Image, ActivityIndicator, Dimensions, Modal } from 'react-native';
...
const GameCardLibrary = ({ navigation }: any) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const resCards = await apiClient.get(`/api/game/cards`);
      const resGame = await apiClient.get(`/api/game/data?email=${email}`);
    ...
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient colors={colors.linearGradient} style={styles.mainContainer}>
        ...
        <ScreenHeader
          ...
          backIcon={<MaterialCommunityIcons name="arrow-left" size={28} color="#FFF" />}
          spacerWidth={28}
        />
