...
import { View, Text, StyleSheet, TextInput, Alert, ScrollView, Image, TouchableOpacity, Platform, ActivityIndicator, Modal } from 'react-native';
...
const CreatePostTab = () => {
  ...
  const handleUserSearch = async (text: string) => {
    ...
    setIsSearching(true);
    try {
      const res = await apiClient.get(`/api/users/search?query=${text}`);
