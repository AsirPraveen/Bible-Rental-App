...
import { View, Text, StyleSheet, TextInput, Alert, ScrollView, TouchableOpacity, Platform, Image, ActivityIndicator, Modal, FlatList, Pressable } from 'react-native';
...
const CreateBookTab = () => {
  ...
  const fetchAuthors = async () => {
    try {
      const res = await apiClient.get(`/api/authors`);
  ...
  const handleCreateAuthor = async () => {
    ...
    try {
      const res = await apiClient.post(`/api/authors`, {
  ...
  const handleAddBook = async () => {
    ...
    try {
      ...
      const response = await apiClient.post(`/api/add-book`, newBook);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const res = await apiClient.delete(`/api/books/${book._id}`);
              if (res.data.status === 'Ok') {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const res = await apiClient.put(`/api/books/${editingBook._id}`, updateData);
  ...
  return (
    <SafeAreaView style={styles.outer_container}>
      ...
      {isSubmitting && (
        <Modal
          navigationBarTranslucent
          statusBarTranslucent transparent={false} animationType="fade">
