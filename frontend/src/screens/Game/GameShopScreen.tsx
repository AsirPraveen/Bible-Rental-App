...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Platform, Alert, Modal, Image, Animated } from 'react-native';
...
const GameShop = ({ navigation }: any) => {
  // This screen is emerald in both themes, so its bars are too.
  useSystemBars({ top: '#064E3B' });
  ...
  const fetchUserData = async () => {
    try {
      ...
      const resUser = await apiClient.post(`/api/auth/userdata`, { token });
      const userEmail = resUser.data.data.email;
      setEmail(userEmail);

      const resGame = await apiClient.get(`/api/game/data?email=${userEmail}`);
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            setLoading(true);
            try {
              const res = await apiClient.post(`/api/game/shop/buy-pack`, {
                email,
                packType
              });
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            setLoading(true);
            try {
              const res = await apiClient.post(`/api/game/shop/buy-armor`, { email });
              if (res.data.status === 'ok') {
                setTalents(res.data.data.talents);
    ...
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient colors={['#064E3B', '#022C22']} style={styles.container}>
        ...
        <ScreenHeader
          ...
          title="The Merchant's Tent"
          backIcon={<MaterialCommunityIcons name="arrow-left" size={28} color="#D1FAE5" />}
          right={
        ...
        <Modal
          ...
          statusBarTranslucent visible={revealIndex !== -1} transparent animationType="fade">
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#064E3B' },
  container: { flex: 1 },
  header: {
    flexDirection: 'row',
