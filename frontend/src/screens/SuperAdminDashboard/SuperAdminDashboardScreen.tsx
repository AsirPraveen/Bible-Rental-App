...
import { View, StyleSheet, Text, TextInput, FlatList, TouchableOpacity, Switch, ActivityIndicator, Alert, Platform, Modal, RefreshControl } from 'react-native';
...
export default function SuperAdminDashboard({ navigation }: any) {
  ...
  const fetchDashboardData = async (isRefresh = false) => {
    try {
      ...
      const [analyticsRes, orgsRes, adminsRes] = await Promise.all([
        apiClient.get(`/api/superadmin/analytics`),
        apiClient.get(`/api/superadmin/organizations`),
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const res = await apiClient.post(`/api/superadmin/demote`, { email });
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setActioning(orgId);
              const res = await apiClient.post(`/api/superadmin/organizations/toggle-status`, { orgId });
              if (res.data.status === 'Ok') {
                await fetchDashboardData();
              }
            } catch (err) {
  ...
  const handlePromoteUser = async () => {
    ...
    try {
      setPromoting(true);
      const res = await apiClient.post(`/api/superadmin/promote`, { email: promoteEmail.trim() });
      if (res.data.status === 'Ok') {
  ...
  const handleCreateOrg = async () => {
    ...
    try {
      setCreatingOrg(true);
      const res = await apiClient.post(`/api/superadmin/organizations/create`, {
