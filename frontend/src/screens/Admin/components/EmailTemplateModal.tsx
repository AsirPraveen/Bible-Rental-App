...
const EmailTemplateModal: React.FC<EmailTemplateModalProps> = ({ isVisible, onClose }) => {
  ...
  const fetchTemplate = async () => {
    setIsLoading(true);
    try {
      const res = await apiClient.get(`/api/email-template/${templateType}`);
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await apiClient.post(`/api/email-template/update`, {
  ...
  return (
    <Modal
      navigationBarTranslucent visible={isVisible} animationType="fade" transparent={true} statusBarTranslucent={true}>
