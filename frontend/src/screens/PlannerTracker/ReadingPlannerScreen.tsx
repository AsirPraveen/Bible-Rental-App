...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Modal, Platform, Alert, type GestureResponderEvent } from 'react-native';
...
import type { ReadingPlan, PlanDay, ChapterReading, BookCursor, PlanDuration } from './types';
...
const generateContinuousReadingPlan = (days: number, scope: string[] = ['Old Testament', 'New Testament']) => {
...
const generateMixedReadingPlan = (days: number, scope: string[] = ['Old Testament', 'New Testament']) => {
  // Create arrays for each book's chapters
  const bookChapters: BookCursor[] = [];
  Object.entries(BIBLE_STRUCTURE).forEach(([testament, books]) => {
    if (scope.includes(testament)) {
      Object.entries(books).forEach(([book, chapters]) => {
        const chaptersArray: ChapterReading[] = [];
        for (let i = 1; i <= chapters; i++) {
  ...
  const totalChapters = bookChapters.reduce((sum: any, b: any) => sum + b.chapters.length, 0);
  const chaptersPerDay = Math.ceil(totalChapters / days);
  const plan: PlanDay[] = [];
  ...
  for (let day = 1; day <= days; day++) {
    const dayReadings: ChapterReading[] = [];
...
const ReadingPlannerComponent = () => {
  ...
  const [selectedDayReading, setSelectedDayReading] = useState<(PlanDay & { planId: string }) | null>(null);
  ...
  const savePlans = async (plans: ReadingPlan[]) => {
    try {
  ...
  const syncReadingStatsToCloud = async (plans: ReadingPlan[]) => {
    try {
      // Calculate total chapters read across all plans
      let totalChaptersRead = 0;
      plans.forEach((p: any) => {
        p.plan.forEach((day: any) => {
          if (day.completed) {
      ...
      if (token) {
        ...
        if (userRes.data && userRes.data.data && userRes.data.data._id) {
          const planProgress = plans.map((p: any) => {
            const completedDays = p.plan.filter((d: any) => d.completed).length;
            const currentDay = Math.min(completedDays + 1, p.days);
            const isCompletedToday = p.plan.find((d: any) => d.day === currentDay)?.completed || false;
          ...
          await apiClient.post(`/api/reading-tracker/sync`, {
  ...
  const selectScope = (scope: any) => {
    setSelectedScope(scope);
    setShowScopeModal(false);
    setShowNewPlanModal(true);
  };

  const selectDuration = (duration: PlanDuration) => {
    setSelectedDuration(duration);
    setShowNewPlanModal(false);
    setShowPlanTypeModal(true);
  };

  const getApproxChaptersPerDay = (duration: PlanDuration) => {
    ...
    Object.entries(BIBLE_STRUCTURE).forEach(([testament, books]) => {
      if (selectedScope.includes(testament)) {
        Object.values(books).forEach((ch: any) => {
          totalChapters += ch;
        });
      }
    });
  ...
  const createNewPlan = (planType: 'mixed' | 'continuous') => {
    const daysMap: Record<PlanDuration, number> = {
      '1 Month': 30, '3 Months': 90, '6 Months': 180, '1 Year': 365,
    };
    const days = selectedDuration ? daysMap[selectedDuration as PlanDuration] : 0;
    ...
    const newPlan = {
      ...
      duration: selectedDuration as PlanDuration,
      scope: selectedScope,
      planType,
      days,
  ...
  const toggleDayComplete = (
    ...
    const updatedPlans = activePlans.map((plan: any) => {
      if (plan.id === planId) {
        const updatedPlan = plan.plan.map((d: any) => 
  ...
  const openReadingModal = (dayPlan: PlanDay, planId: string) => {
    setSelectedDayReading({ ...dayPlan, planId });
    setShowReadingModal(true);
  };

  const deletePlan = (planId: any) => {
    Alert.alert(
      'Delete Plan',
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            const updated = activePlans.filter((p: any) => p.id !== planId);
            savePlans(updated);
  ...
  const getDaysToShow = (plan: ReadingPlan) => {
    const completedDays = plan.plan.filter((d: any) => d.completed).length;
  ...
  const groupReadingsByTestament = (readings: ChapterReading[]) => {
    const grouped: Record<string, ChapterReading[]> = {
      'Old Testament': [],
      'New Testament': [],
    };
                          ...
                          <View style={styles.dayInfo}>
                            ...
                            <View style={styles.dayTextContainer}>
                              ...
                              <Text style={styles.readingText} numberOfLines={2}>
                                {dayPlan.readings.slice(0, 3).map((r: any) => `${r.book} ${r.chapter}`).join(', ')}
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.readingModalContainer}>
              ...
              <View style={styles.readingModalFooter}>
                {(() => {
                  const plan = activePlans.find((p: any) => p.id === selectedDayReading?.planId);
                  const completedDays = plan?.plan.filter((d: any) => d.completed).length || 0;
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              ...
              {['1 Month', '3 Months', '6 Months', '1 Year'].map((duration: any) => (
                <TouchableOpacity
                  key={duration}
